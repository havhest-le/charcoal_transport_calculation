###
# Script to select a buffer of 100 km around the lakes Khamra and Satagay
###
...
# Reclassifying vegetation of boreal forest
reclass_df                              <- c(1, 2, 1, 2, 4, 2)
reclass_m                               <- matrix(reclass_df, ncol = 3, byrow = TRUE)
veget_classified                        <- reclassify(veget, reclass_m)
...
# Mapping and saving study area | Yakutia #
png(glue("Results/study_area_yakutia_new.png"), width = 1000, height = 700)
...
plot(veget_classified,
     ...
     col = colors,
     box = FALSE,
     main = "Study area Yakutia | Russia", cex.main = 2.5)
...
# Mapping single lakes with 100 km buffer #
