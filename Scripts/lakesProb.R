...
map0     <- read_sf("Data/ne_10m_admin_1_states_provinces/ne_10m_admin_1_states_provinces.shp") %>%
  st_shift_longitude() # All longitudes < 0 are added to 360

map  <- map0[map0$adm1_code %in% c("RUS-2612", "RUS-2615", "RUS-2321"),] %>% st_geometry() # Get, set, or replace geometry from an sf object
...
lakes   <- read_sf("Data/lakes_sf.shp") 


ext <- as(extent(st_bbox(lakes %>% st_buffer(250000) %>% st_union() %>% st_transform(4326) %>% st_shift_longitude())[c(1,3,2,4)]), "SpatialPolygons")


### files
files <- list.files(path = "Z:/bioing/data/Data_Reanalyse/ERA5/Wind_u_v_pressureLevels", pattern = "*.nc", all.files = T, full.names = T)

fls_Tab <- do.call("rbind", lapply(files, function(x) {
  nf <- nc_open(x)
  tms <- as.POSIXct(nf$var[[1]]$dim[[4]]$vals*60*60, origin = "1990-01-01")
